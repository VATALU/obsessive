...
public final class BeanFactory {
    ...
    public BeanFactory(Set<Class<?>> beanClassSet) {
        ...
        beanClassSet.forEach(beanClass -> {
            Object obj = Reflections.instance(beanClass);
                    ...
                    for (Field beanField : beanFields) {
                        ...
                        if (beanField.isAnnotationPresent(Inject.class)) {
                            ...
                            if (beanFieldInstance != null) {
                                // 通过反射初始化BeanField的值
                                Reflections.setField(beanInstance, beanField, beanFieldInstance);
                            }
                        }
                        // @Value 的 Field
                        if (beanField.isAnnotationPresent(Value.class)) {
                            Reflections.setField(beanInstance, beanField, beanField.getAnnotation(Value.class).value());
                        }
                    }
                }
            }
